...
import Compile.Warning ( Warning(..) )
...
import Compile.Compile (CompInfo(..))
...
import Feedback.Analyse (hasRedundantPattern, missingBaseCase)
...
mkHoleFeedback s m = case getMatchLocs (core s) (core m) of
    ...
    where vars = translateNames (names s) (names m)
          mkSugg = map (mkMatches (parsed m) vars)
...
printHoleMatch s m = strings
        where holematches = getMatchLocs (core s) (core m)
              nameMap = translateNames (names s) (names m)
              strings = map (showTransParsedFromLoc nameMap (parsed m)) holematches 
